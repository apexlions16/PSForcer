...
bool JsonParser::parse(const std::string& text, JsonValue& output, std::string& error) {
    ...
    skipWhitespace();
    if (position_ != text_->size()) return fail("JSON verisinin sonunda beklenmeyen içerik var");
    return true;
}
...
bool JsonParser::parseValue(JsonValue& value) {
    ...
    if (c == 't') {
        ...
        value = JsonValue(); value.type_ = JsonValue::Type::Boolean; value.bool_ = true; return true;
    }
    if (c == 'f') {
        if (!matchLiteral("false")) return false;
        value = JsonValue(); value.type_ = JsonValue::Type::Boolean; value.bool_ = false; return true;
    }
    if (c == 'n') {
        if (!matchLiteral("null")) return false;
        value = JsonValue(); return true;
    }
    return fail("JSON değeri bekleniyordu");
}

bool JsonParser::parseObject(JsonValue& value) {
    if (get() != '{') return fail("Nesne bekleniyordu");
    value = JsonValue(); value.type_ = JsonValue::Type::Object;
    skipWhitespace();
    ...
    while (true) {
        ...
        skipWhitespace();
        if (get() != ':') return fail("Nesne anahtarından sonra ':' bekleniyordu");
        JsonValue child;
        ...
        if (separator != ',') return fail("Nesnede ',' veya '}' bekleniyordu");
    }
}

bool JsonParser::parseArray(JsonValue& value) {
    if (get() != '[') return fail("Dizi bekleniyordu");
    value = JsonValue(); value.type_ = JsonValue::Type::Array;
    skipWhitespace();
    if (peek() == ']') { get(); return true; }
    while (true) {
        JsonValue child;
        ...
        const char separator = get();
        if (separator == ']') return true;
        if (separator != ',') return fail("Dizide ',' veya ']' bekleniyordu");
    }
}

bool JsonParser::parseString(std::string& value) {
    if (get() != '"') return fail("JSON metni bekleniyordu");
    value.clear();
    while (position_ < text_->size()) {
        char c = get();
        if (c == '"') return true;
        if (static_cast<unsigned char>(c) < 0x20) return fail("JSON metninde denetim karakteri var");
        if (c != '\\') { value.push_back(c); continue; }
        if (position_ >= text_->size()) return fail("JSON kaçış dizisi tamamlanmamış");
        ...
        switch (escaped) {
            ...
            case 'u': {
                if (position_ + 4 > text_->size()) return fail("Evrensel karakter kaçışı tamamlanmamış");
                unsigned int codepoint = 0;
                for (int i = 0; i < 4; ++i) {
                    const int digit = hexDigit(get());
                    if (digit < 0) return fail("Evrensel karakter kaçışı geçersiz");
                    codepoint = (codepoint << 4) | static_cast<unsigned int>(digit);
                }
                appendUtf8(value, codepoint);
                break;
            }
            default: return fail("JSON kaçış dizisi geçersiz");
        }
    }
    return fail("JSON metni tamamlanmamış");
}
...
bool JsonParser::parseNumber(JsonValue& value) {
    ...
    if (peek() == '0') get();
        ...
        if (peek() < '1' || peek() > '9') return fail("JSON sayısı geçersiz");
        while (peek() >= '0' && peek() <= '9') get();
    }
    if (peek() == '.') {
        get();
        if (peek() < '0' || peek() > '9') return fail("JSON ondalık sayısı geçersiz");
        while (peek() >= '0' && peek() <= '9') get();
    }
    if (peek() == 'e' || peek() == 'E') {
        get();
        if (peek() == '+' || peek() == '-') get();
        if (peek() < '0' || peek() > '9') return fail("Üs değeri geçersiz");
        while (peek() >= '0' && peek() <= '9') get();
    }
    ...
    if (errno != 0 || end == token.c_str() || *end != '\0' || !std::isfinite(parsed)) return fail("JSON sayısı geçersiz");
    value = JsonValue(); value.type_ = JsonValue::Type::Number; value.number_ = parsed;
    return true;
}
...
bool JsonParser::matchLiteral(const char* literal) {
    ...
    while (*literal) {
        if (get() != *literal++) {
            position_ = start;
            return fail("JSON sabit değeri geçersiz");
        }
    }
    return true;
}
...
bool JsonParser::fail(const std::string& message) {
    ...
    stream << message << "; bayt konumu: " << position_;
    *error_ = stream.str();
    return false;
}
